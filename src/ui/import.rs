...
impl State {
    ...
    pub fn update(&mut self, message: Message, config: &Config) -> Task<Message> {
        match message {
            Message::SelectSource => Task::perform(
                async {
            ...
            Message::SourceSelected(path) => {
                if let Some(p) = path {
                    ...
                    Task::perform(
                        async move { Importer::scan_directory(&p) },
            ...
            Message::StartImport => {
                if let Some(to_sort) = &config.to_sort_dir {
                    let selected_photos: Vec<Photo> = self
                        .photos_to_import
                        .iter()
                    ...
                    self.status =
                        "Please configure a 'To Sort' directory in Settings first.".to_string();
                    Task::none()
                }
            }
    ...
    fn update_status(&mut self) {
        ...
        self.status = format!(
            "Found {} photos. {} selected. Ready to import.",
            total, selected
        );
    }
    ...
    pub fn view(&self) -> Element<'_, Message> {
        ...
        if !self.photos_to_import.is_empty() && !self.is_importing {
            import_info = import_info.push(
                button(
                    text(format!("START IMPORT ({}/{})", selected_count, total_count))
                        .font(bold_font()),
                )
        ...
        let mut main_col =
            column![text("IMPORT PHOTOS").font(bold_font()).size(28), card].spacing(20);

        if !self.photos_to_import.is_empty() {
            let mut filetypes: Vec<String> = self
                .photos_to_import
                .iter()
                .filter_map(|(photo, _)| {
                    photo
            ...
            for (idx, (photo, selected)) in self.photos_to_import.iter().enumerate() {
                ...
                let camera_str = photo
                    .camera_model
                    .as_ref()
                    .map(|c| c.to_uppercase())
                    .unwrap_or_else(|| "NO EXIF CAMERA".to_string());
                let date_str = photo
                    .date_taken
                    .as_ref()
                    .map(|d| d.format("%Y-%m-%d %H:%M:%S").to_string())
                    .unwrap_or_else(|| "NO EXIF DATE".to_string());
                ...
                let row_item = container(
                    row![
                        ...
                        column![
                            text(relative_path.to_uppercase())
                                .font(bold_font())
                                .size(14),
                            row![
            ...
            let list_header = row![
                ...
                button(
                    text(if self.is_filetype_dropdown_open {
                        "FILETYPE ▴"
                    } else {
                        "FILETYPE ▾"
                    })
            ...
            let mut list_card_contents = column![list_header,].spacing(8);
